...
import mcen.registry.IRegister
...
abstract class NetworkRegistry : IRegister {
    ...
    override fun register(modId: String, modBus: IEventBus, forgeBus: IEventBus) {
        ...
        modBus.addListener { _: FMLCommonSetupEvent ->
            instance = NetworkRegistry.newSimpleChannel(ResourceLocation(modId, name), { "1.0" }, { true }, { true })
            for (packet in listeners) {
                val packetClass = packet.key
                val listener = packet.value
                instance.messageBuilder(packetClass as Class<Packet>, packet.value.id).encoder { pkt, buffer ->
                        pkt.write(buffer)
                    }.decoder {
    ...
    protected inline fun <reified T : Packet> register(
        id: Int, noinline packetSupplier: () -> T
    ...
    fun sendToClientsWithBlockLoaded(
        packet: Packet, blockPos: BlockPos, world: Level
     ...
     */
    fun <T : Packet> sendToServer(packet: T) = instance.sendToServer(packet)
    ...
    fun NetworkEvent.Context.sendToClient(packet: Packet) {
        instance.sendTo<Any>(
            packet, this.networkManager, NetworkDirection.PLAY_TO_CLIENT
        )
    }
}